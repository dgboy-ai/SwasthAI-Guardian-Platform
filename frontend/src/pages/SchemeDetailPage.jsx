...
import {
  Stethoscope, Baby, Heart, Shield, Apple, Pill,
  ClipboardList, AlertTriangle, DollarSign, User,
  MapPin, Lightbulb, FileText, Phone, Globe, Copy, CheckCircle,
  ArrowLeft, ExternalLink, Fingerprint, Package, Building2,
  Camera, Home
} from 'lucide-react';

// ── Category meta ─────────────────────────────────────────────────────────────
const CATEGORY_META = {
  health_insurance: { icon: Stethoscope, color: '#10b981', bg: '#d1fae5', border: '#6ee7b7', glow: 'rgba(16,185,129,0.15)' },
  maternal_health:  { icon: Heart, color: '#8b5cf6', bg: '#ede9fe', border: '#c4b5fd', glow: 'rgba(139,92,246,0.15)' },
  child_health:     { icon: Baby, color: '#f59e0b', bg: '#fef3c7', border: '#fcd34d', glow: 'rgba(245,158,11,0.15)' },
  insurance:        { icon: Shield, color: '#3b82f6', bg: '#dbeafe', border: '#93c5fd', glow: 'rgba(59,130,246,0.15)' },
  nutrition:        { icon: Apple, color: '#ef4444', bg: '#fee2e2', border: '#fca5a5', glow: 'rgba(239,68,68,0.15)' },
  disease:          { icon: Pill, color: '#06b6d4', bg: '#cffafe', border: '#67e8f9', glow: 'rgba(6,182,212,0.15)' },
  other:            { icon: ClipboardList, color: '#6b7280', bg: '#f3f4f6', border: '#d1d5db', glow: 'rgba(107,114,128,0.1)' },
};
...
function CopyBtn({ text, t }) {
  ...
  return (
    ...
    >
      {copied ? (t?.schemes?.copied || 'Copied!') : <Copy size={14} />}
    </button>
  );
}
...
export default function SchemeDetailPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #f0fdf4, #f8faff 60%, #fef9ff)', fontFamily: "'Inter','Segoe UI',sans-serif", paddingBottom: 100 }}>
      ...
      <div ref={headerRef} style={{ background: `linear-gradient(135deg, ${cat.color}18 0%, ${cat.color}08 100%)`, borderBottom: `1px solid ${cat.border}`, padding: '24px 20px 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <Link to="/schemes" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: cat.color, fontWeight: 700, textDecoration: 'none', marginBottom: 16, background: cat.bg, padding: '6px 14px', borderRadius: 20, border: `1px solid ${cat.border}` }}>
            <ArrowLeft size={14} /> {s.back || 'Back to Schemes'}
          </Link>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12, fontWeight: 800, color: cat.color, background: cat.bg, padding: '4px 14px', borderRadius: 20, border: `1px solid ${cat.border}`, textTransform: 'uppercase', letterSpacing: 0.8 }}>
              <cat.icon size={12} style={{ display: 'inline' }} /> {cat.label}
            </span>
            <span style={{ fontSize: 11, color: '#10b981', fontWeight: 700, background: '#d1fae5', padding: '3px 10px', borderRadius: 20, border: '1px solid #6ee7b7' }}>
              <CheckCircle size={12} style={{ display: 'inline' }} /> {s.active_scheme || 'Active Scheme'}
            </span>
            {scheme.start_year && (
          ...
          <div style={{ background: `linear-gradient(135deg, ${cat.color}, ${cat.color}bb)`, borderRadius: '14px 14px 0 0', padding: '16px 22px', marginTop: 16, display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ fontSize: 28, display: 'flex', alignItems: 'center' }}><DollarSign size={28} color="#fff" /></div>
            <div>
      ...
      {/* Content */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }} style={{ maxWidth: 900, margin: '0 auto', padding: '20px 16px' }}>

        {/* Why This Helps You */}
        {whyHelps && (
          <Section title={<><Heart size={16} style={{ display: 'inline', marginRight: 6, verticalAlign: 'middle' }} />{s.why_helps_title || 'Why This Helps You'}</>} accentColor={cat.color}>
        ...
        {/* Eligibility */}
        <Section title={<><CheckCircle size={16} style={{ display: 'inline', marginRight: 6, verticalAlign: 'middle' }} />{s.eligibility_title || 'Are You Eligible?'}</>} accentColor="#3b82f6">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            <EligCard icon={<User size={18} />} label={s.eligibility_age || 'Age'} value={`${scheme.min_age}–${scheme.max_age === 120 ? 'Any age' : scheme.max_age} ${s.age_range || 'years'}`} />
            <EligCard icon={<><User size={18} /><User size={18} /></>} label={s.eligibility_gender || 'Who Can Apply'} value={genderLabel} />
            <EligCard icon={<ClipboardList size={18} />} label={s.eligibility_bpl || 'BPL Required?'} value={bplLabel} highlight={scheme.economic_status_eligibility === 'BPL'} />
            <EligCard icon={<MapPin size={18} />} label={s.eligibility_area || 'Area'} value={areaLabel} />
          </div>
          <div style={{ marginTop: 14, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#78350f', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            <Lightbulb size={16} style={{ flexShrink: 0, color: '#78350f' }} />
        ...
        {/* Documents */}
        {documents.length > 0 && (
          <Section title={<><FileText size={16} style={{ display: 'inline', marginRight: 6, verticalAlign: 'middle' }} />{s.documents_title || 'Documents Needed'}</>} accentColor="#f59e0b">
        ...
        {/* How to Apply — Steps */}
        {steps.length > 0 && (
          <Section title={<><MapPin size={16} style={{ display: 'inline', marginRight: 6, verticalAlign: 'middle' }} />{s.how_to_apply || 'How to Apply — Step by Step'}</>} accentColor="#8b5cf6">
          ...
          {scheme.helpline && (
            <a href={`tel:${scheme.helpline.replace(/[^0-9+]/g, '')}`} style={{ textDecoration: 'none' }}>
              <div style={{ background: '#fff', borderRadius: 14, padding: '18px 20px', border: '1px solid #e5e7eb', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', display: 'flex', gap: 14, alignItems: 'center', cursor: 'pointer', transition: 'box-shadow 0.2s' }}>
                <div style={{ width: 46, height: 46, borderRadius: 12, background: 'linear-gradient(135deg, #10b981, #059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Phone size={22} color="#fff" /></div>
                <div>
          ...
          {scheme.official_url && (
            <a href={scheme.official_url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
              <div style={{ background: `linear-gradient(135deg, ${cat.color}, ${cat.color}cc)`, borderRadius: 14, padding: '18px 20px', display: 'flex', gap: 14, alignItems: 'center', cursor: 'pointer', boxShadow: `0 4px 16px ${cat.glow}` }}>
                <div style={{ fontSize: 28, flexShrink: 0, display: 'flex', alignItems: 'center' }}><Globe size={28} color="#fff" /></div>
                <div>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#fff', lineHeight: 1.3 }}>{s.official_portal || 'Apply on Official Website'} <ExternalLink size={14} style={{ display: 'inline' }} /></div>
        ...
        <div style={{ background: 'linear-gradient(135deg, #065f46, #047857)', borderRadius: 16, padding: '18px 22px', display: 'flex', gap: 14, alignItems: 'center', color: '#fff' }}>
          <div style={{ fontSize: 30, flexShrink: 0, display: 'flex', alignItems: 'center' }}><Stethoscope size={30} color="#fff" /></div>
          <div>
...
function getDocIcon(doc) {
  ...
  if (d.includes('aadhaar') || d.includes('aadhar')) return <Fingerprint size={20} />;
  if (d.includes('ration')) return <Package size={20} />;
  if (d.includes('bank') || d.includes('account')) return <Building2 size={20} />;
  if (d.includes('birth')) return <Baby size={20} />;
  if (d.includes('income') || d.includes('bpl')) return <DollarSign size={20} />;
  if (d.includes('mcp') || d.includes('mother')) return <ClipboardList size={20} />;
  if (d.includes('photo')) return <Camera size={20} />;
  if (d.includes('phone') || d.includes('mobile')) return <Phone size={20} />;
  if (d.includes('address')) return <Home size={20} />;
  if (d.includes('age') || d.includes('proof')) return <FileText size={20} />;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0fdf4', fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{ textAlign: 'center', padding: '80px 20px' }}>
        <div style={{ fontSize: 52 }}><AlertTriangle size={52} style={{ color: '#ef4444' }} /></div>
        ...
        <Link to="/schemes" style={{ display: 'inline-block', marginTop: 12, padding: '10px 24px', background: '#10b981', color: '#fff', borderRadius: 10, textDecoration: 'none', fontWeight: 800 }}>
          <><ArrowLeft size={14} style={{ display: 'inline' }} /> {s.back || 'Back to Schemes'}</>
        </Link>
      </div>
    </div>
  );
