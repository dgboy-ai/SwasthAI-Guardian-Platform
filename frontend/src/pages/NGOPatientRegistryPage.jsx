...
export default function NGOPatientRegistryPage() {
  ...
  const paginatedPatients = filtered.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
            ...
            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                currentPage === 1
                  ...
                  : 'bg-white text-slate-700 border border-slate-200 hover:border-slate-300 hover:shadow-sm active:scale-95'
              }`}>
              ← Prev
            </button>
