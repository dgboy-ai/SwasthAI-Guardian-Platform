...
const metrics = [
  ...
  { label: 'Symptoms Checked Today', value: 18, icon: Stethoscope, color: '#059669', bg: '#ECFDF5', change: 'Today' },
...
export default function LiveFieldImpact({ className = '' }) {
  return (
    <div className={`bg-white border border-slate-100 rounded-3xl p-5 shadow-xs ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
        {metrics.map((m) => {
          const Icon = m.icon;
          return (
            <div key={m.label} className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-left hover:shadow-sm transition-shadow">
