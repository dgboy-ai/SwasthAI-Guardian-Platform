...
import { AlertTriangle, MapPin, Thermometer, ClipboardCheck, Shield, Activity, Radio, Eye, ClipboardList, CheckCircle } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1.5 border-b border-slate-100 pb-2">
        {['villages', 'actions', 'checklist'].map(tab => (
          ...
          >
            {tab === 'villages' ? <><MapPin className="w-3.5 h-3.5" /> Villages</> : tab === 'actions' ? <><ClipboardList className="w-3.5 h-3.5" /> Actions</> : <><CheckCircle className="w-3.5 h-3.5" /> Checklist</>}
          </button>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'villages' && (
          <motion.div key="villages" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }} className="space-y-2">
        ...
        {activeTab === 'actions' && (
          <motion.div key="actions" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }} className="bg-emerald-50 border border-emerald-100 rounded-2xl p-3.5 space-y-2">
        ...
        {activeTab === 'checklist' && (
          <motion.div key="checklist" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }} className="space-y-1.5">
